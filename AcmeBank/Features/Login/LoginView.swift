...
struct LoginView: View {
    ...
    var body: some View {
        VStack(spacing: 0) {
            ...
            // Dismiss the keyboard the moment the user begins a scroll
            // gesture. This is good UX (matches Mail / Settings) AND it
            // gives XCUITest a deterministic way to dismiss the IME so the
            // Sign In button at y≈562 — which sits beneath the keyboard
            // once the password SecureField has been focused on iPhone 16 —
            // is reachable. Without this, tapping a non-interactive Text
            // ("Acme Bank") does NOT resign first responder in SwiftUI, so
            // the keyboard stays up and the tap reports hit point {-1, -1}
            // / "Not hittable".
